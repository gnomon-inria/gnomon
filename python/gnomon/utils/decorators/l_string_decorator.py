...
from gnomon.core import gnomonLString
...
form_data_factory = gnomon.core.lStringData_pluginFactory()
...
def lStringInput(attr, data_plugin, methods=(None, None)):
    """
    Implements the accessor methods to attr for an l-string input.
    Those accessors will convert between a gnomonLString and the type
...
def lStringOutput(attr, data_plugin, method=None):
    """
    Implements the getter method to attr for an l-string output.
    This getter will convert between a gnomonLString and the type
    of attr as defined by the data_plugin.
