...
def dataFrameInput(attr, data_plugin, methods=(None, None)):
    """
    Implements the accessor methods to attr for a data frame input.
    Those accessors will convert between a gnomonDataFrameSeries and the type
    ...
    data_plugin: Union[type, str]
        ...
        e.g. gnomonDataFrameDataPandas --> DataFrame (from pandas)
    methods: (str, str)
...
def dataFrameOutput(attr, data_plugin, method=None):
    """
    Implements the getter method to attr for a data frame output.
    This getter will convert between a gnomonDataFrameSeries and the type
    of attr as defined by the data_plugin.
