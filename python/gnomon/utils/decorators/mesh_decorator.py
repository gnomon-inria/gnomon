...
def meshInput(attr, data_plugin, methods=(None, None)):
    """
    Implements the accessor methods to attr for a mesh input.
    Those accessors will convert between a gnomonMeshSeries and the type
    ...
    data_plugin: Union[type, str]
        ...
        e.g. gnomonMeshDataPropertyTopomesh --> PropertyTopomesh (from cellcomplex)
    methods: (str, str)
...
def meshOutput(attr, data_plugin, method=None):
    """
    Implements the getter method to attr for a mesh output.
    This getter will convert between a gnomonMeshSeries and the type
    of attr as defined by the data_plugin.
