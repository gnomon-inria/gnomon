...
gnomonMainWindow::gnomonMainWindow(QWidget *parent) : dtkWidgetsMainWindow(parent)
{
    ...
    d->workspace_bar->addWorkspaceInMenu("Tissue Forms", tissue_package_desc, "Cell Image Analysis", "Carries out quantitative analyzes of multicellular tissues", "Cell Image Analysis", gnomonWorkspaceCellImageFilter::color);
    d->workspace_bar->addWorkspaceInMenu("Tissue Forms", tissue_package_desc, "Cell Image Meshing", "Provides plugins to mesh the cells of a multicellular tisue", "Cell Image Meshing", gnomonWorkspaceCellComplexFromCellImage::color);
    d->workspace_bar->addWorkspaceInMenu("Tissue Forms", tissue_package_desc, "Cell Image Morpho Filter", "Provides various classsical morphological filters to process labelled images", "Cell Image Morpho Filter", gnomonWorkspaceCellImageFilter::color);
    const QString detection_desc = "Detects cells in a 3D intensity image of tissue as a 3D point cloud.";
    d->workspace_bar->addWorkspaceInMenu("Tissue Forms", tissue_package_desc, "Image Cell Detection", "Provides plugins to detect cells of cell nuclei in intensity images", "Image Cell Detection", gnomonWorkspacePointCloudFromImage::color);
    d->workspace_bar->addWorkspaceInMenu("Tissue Forms", tissue_package_desc, "Image Fusion", "Fuses images that have been taken of an object from different angles", "Image Fusion", gnomonWorkspaceFusion::color);
    d->workspace_bar->addWorkspaceInMenu("Tissue Forms", tissue_package_desc, "Image Preprocessing",   "Provides various plugins to pre-process (noise, signal enhancement,...) intensity images", "Image Preprocessing", gnomonWorkspacePreprocess::color);
    const QString segmentation_desc = "Performs segmentation of 3D intensity images at cellular resolution.";
    d->workspace_bar->addWorkspaceInMenu("Tissue Forms", tissue_package_desc, "Image Segmentation",    "Provides plugins to segment intensity images of multicellular tissues", "Image Segmentation", gnomonWorkspaceSegmentation::color);
    d->workspace_bar->addWorkspaceInMenu("Tissue Forms", tissue_package_desc, "Image Surface Meshing", "Provides plugins to mesh the surface of a tissue based on intensity images", "Image Surface Meshing", gnomonWorkspaceMeshFromImage::color);
