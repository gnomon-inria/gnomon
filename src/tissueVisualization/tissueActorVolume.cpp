...
// /////////////////////////////////////////////////////////////////
// tissueActorVolumePrivate
// /////////////////////////////////////////////////////////////////

class tissueActorVolumePrivate
{
public:
    vtkImageData *volume;
...
vtkStandardNewMacro(tissueActorVolume);

void tissueActorVolume::setVolume(vtkImageData *volume)
{
    d->volume = volume;
...
void tissueActorVolume::setInteractor(void *interactor)
{
    d->interactor = static_cast<vtkRenderWindowInteractor *>(interactor);
}

void tissueActorVolume::update(void)
{
    if(!d->volume)
        return;
...
void tissueActorVolume::showScalarBarTitle(bool show)
{
    if (!d->scalarBar)
        return;
...
void tissueActorVolume::setScalarBarOrientationToVertical(bool value)
{
    if (d->scalarBar) {
        if (value) {
...
void tissueActorVolume::outlineNone(void)
{
    d->outline_corner_actor->SetVisibility(0);
    d->outline_box_actor->SetVisibility(0);
    d->outline_contour_actor->SetVisibility(0);
...
void tissueActorVolume::outlineCorners(void)
{
...
void tissueActorVolume::outlineBox(void)
{
    d->outline_corner_actor->SetVisibility(0);
...
void tissueActorVolume::outlineContour(void)
{
    d->outline_corner_actor->SetVisibility(0);
    d->outline_box_actor->SetVisibility(0);
...
void *tissueActorVolume::colorTransferFunction(void)
{
    return d->colorFunction;
}

void *tissueActorVolume::opacityTransferFunction(void)
{
    return d->opacityTransferFunction;
}

void *tissueActorVolume::mapper(void)
{
    return d->mapper;
}

void *tissueActorVolume::vol(void)
{
    return d->vol;
}

void *tissueActorVolume::volumeProperty(void)
{
    return d->volProperty;
}

void tissueActorVolume::setColorTransferFunction(vtkColorTransferFunction *func)
{
    d->colorFunction = func;
    this->update();
}

void tissueActorVolume::setOpacityTransferFunction(vtkPiecewiseFunction *func)
{
    d->opacityTransferFunction = func;
    this->update();
}

tissueActorVolume::tissueActorVolume(void) : tissueActor(), d(new tissueActorVolumePrivate)
{
    d->volume = NULL;
    d->interactor = NULL;
    d->filter = NULL;
...
tissueActorVolume::~tissueActorVolume(void)
{
    delete d;
...
void tissueActorVolume::showScalarBar(bool show)
{
...
//
// tissueActorVolume.cpp ends here
