...
#include "gnomonActor/gnomonLandmarkActor.h"
...
public:
    ...
    gnomonOverlayButton *picker = nullptr;
...
public:
    ...
    gnomonOverlayButton *picker = nullptr;
    gnomonOverlayButton *blending = nullptr;
    gnomonOverlayButton *sync = nullptr;
    gnomonOverlayButton *stack = nullptr;
    QList<gnomonOverlayButton *> layers;
    gnomonOverlayButton *clut = nullptr;
...
gnomonViewVolumicPrivate::gnomonViewVolumicPrivate(QWidget *parent) : QVTKOpenGLWidget(parent)
{
    ...
    this->picker = new gnomonOverlayButton(fa::crosshairs, "", this);
    this->picker->toggle(false);
    this->blending = new gnomonOverlayButton(fa::adjust, "", this);
    this->blending->toggle(false);
    this->sync = new gnomonOverlayButton(fa::unlock, "", this);
    this->sync->toggle(false);
    this->stack = new gnomonOverlayButton(fa::eye, "", this);
    this->stack->toggle(false);
    this->clut = new gnomonOverlayButton(fa::aligncenter, "", this);
    ...
    connect(this->picker, &gnomonOverlayButton::iconClicked, [=] () {
    ...
    connect(this->blending, &gnomonOverlayButton::iconClicked, [=] () {
    ...
    connect(this->sync, &gnomonOverlayButton::iconClicked, [=] () {
    ...
    connect(this->stack, &gnomonOverlayButton::iconClicked, [=] () {
    ...
    connect(this->clut, &gnomonOverlayButton::iconClicked, [=] () {
...
void gnomonViewVolumic::dropEvent(QDropEvent *event)
{
    ...
    if (d->images_serie)
    {
        ...
        for(const QString& layer : layer_names) {
            gnomonOverlayButton *layer_overlay = new gnomonOverlayButton(fa::eye, layer, this);
            ...
            connect(layer_overlay, &gnomonOverlayButton::iconClicked, [=] () {
                if(!layer_overlay->text().isEmpty())
                    d->toggleChannel(layer_overlay->text());
                });
            connect(layer_overlay, &gnomonOverlayButton::textClicked, [=] () {
                 if(!layer_overlay->text().isEmpty())
